package com.member.view;

import java.text.SimpleDateFormat;
import java.util.Date;
import java.util.List;
import java.util.Scanner;

import com.member.controller.MemberController;
import com.member.model.vo.Member;

//view클래스는 기능을 수행할 때 controller를 호출

public class MemberView {

	// 사용자에게 보여질 화면을 구성하는 클래스
	// 화면(콘솔)에 출력되는 메소드(기능)을 가지고 있음 
	// 나중에 화면은 html, css, js, jq ... 
	
	private MemberController mc;
	
	public void mainMenu(MemberController mc) {
		
		this.mc = mc;
		
		Scanner sc = new Scanner(System.in);
		
		System.out.println("==== 회원 관리 프로그램 v.1 ====");
		
		while(true) {
			
			System.out.println("1. 전체회원 조회하기");
			System.out.println("2. 이름으로 조회하기");
			System.out.println("3. 아이디로 조회하기");
			System.out.println("4. 회원등록하기");
			System.out.println("5. 회원정보 수정하기");
			System.out.println("6. 회원삭제하기");
			System.out.println("0. 프로그램 종료");
			System.out.print("메뉴 번호 선택 : ");
			int cho = sc.nextInt();
			
			switch(cho) {
				case 1:
					mc.selectAll();
				break;
				case 2:
					searchName();
				break;
				case 3:
					mc.searchId();
				break;
				case 4:
					mc.insertMember();
				break;
				case 5:
					mc.updateMember();
				break;
				case 6:
					
				break;
				case 0:
					System.out.println("프로그램을 종료합니다");
				return;
					
				
			}
		}
		
	}
	
	public String searchId() {
			
			System.out.println("---- 아이디로 조회하기 ----");
			
			Scanner sc = new Scanner(System.in);
			
			System.out.print("조회할 아이디 : ");
			
			return sc.next();
			
		}
	
	public void searchName() {
		
		System.out.println("---- 이름으로 조회하기 ----");
		
		Scanner sc = new Scanner(System.in);
		
		System.out.print("조회할 이름( 부분 검색 가능 ) : ");
		
		String name = sc.next();
		
		 mc.searchName(name);
	}
	
	
	// 조회하기 메뉴를 하나로 통합할 수 있다
	public String inputData(String keyword) {
		
		Scanner sc = new Scanner(System.in);
		
		System.out.print("---- 검색할 " + keyword + " 입력 ----");
		
		return sc.next();
		
	}

	
	// 콘솔창에 전달받은 데이터를 출력해주는 메소드
	public void printData(List<Member> list) {
		
		for(Member m : list) {
			
			System.out.println(m);
		}
		
	}
	
	public void printData(Member m) {
		
		System.out.println(m);
	}
	
	
	

	public Member inputMember() {
		
		System.out.println("---- 회원 등록 ----");
		
		Scanner sc = new Scanner(System.in);
		
		System.out.print("회원 아이디 : ");
			String memberId = sc.next();
		
		System.out.print("회원 비밀번호 : ");
			String memberPwd = sc.next();
		
		System.out.print("회원 이름 : ");
			String memberName = sc.next();
		
		System.out.print("성별 ( M / F ) : ");
			String gender = sc.next();
			
		System.out.print("나이 : ");
			int age = sc.nextInt();
			
		System.out.print("이메일 : ");
			String email = sc.next();
			
		System.out.print("전화번호( - 제외 ) : ");
			String phone = sc.next();
			
			sc.nextLine();
			
		System.out.print("주소 : ");
			String address = sc.nextLine();
			
		System.out.print("취미( 여러개이면 띄어쓰기 없이 ,로 연결 ) : ");
			String hobby = sc.next();
		
//		System.out.print("가입 날짜 (yyyy-mm-dd 형식으로 ) : ");
//			Date today = new Date();
//			SimpleDateFormat sdf = new SimpleDateFormat("yyyy-mm-dd");
//			// Date enrollDate = sdf.parse(today);
			

		
		return new Member(memberId, memberPwd, memberName, gender, age, email, phone, address, hobby, null);
	}

	
	public void printMsg(String msg) {
		
		System.out.println("---- 시스템 알림 ----");
		
		System.out.println(msg);
		
		System.out.println("------------------");
	}
	
	
	
	public String[] updateMember() {
		
		String column ="";
		String newPw ="";
		String newPhone = "";
		String newAddress = "";
		String[] str = new String[3];
		
		
		System.out.println("---- 회원정보 수정하기 ----");
		
		Scanner sc = new Scanner(System.in);
		
		System.out.print("수정할 회원의 아이디 : ");
			String id = sc.next();
		
		System.out.println("1. 비밀번호 수정");
		System.out.println("2. 전화번호 변경");
		System.out.println("3. 주소 변경");
		
		System.out.print("번호 입력 : ");
			int choice = sc.nextInt();
		
		
		// 수정할 컬럼들 : 비밀번호 주소 핸드폰번호 
		// 컬럼명도 같이 넘겨주면 dao에서 sql을 작성할 때 동적으로 바꿀수 있다 
		
		switch (choice) {
			case 1:
			
				column = "MEMBER_PWD";
				
				System.out.println("새로운 비밀번호 : ");
					newPw = sc.next();
					
				str[0] = id;
				str[1] = newPw;
				str[2] = column;
					
			break;
			case 2:
				
				column = "PHONE";
				
				System.out.println("새로운 전화번호 ( - 제외 ) : ");
					newPhone = sc.next();
					
				str[0] = id;
				str[1] = newPhone;
				str[2] = column;
					
			break;
			case 3:
				
				column = "ADDRESS";
				
				System.out.println("새로운 주소 : ");
					newAddress = sc.next();
				
				str[0] = id;
				str[1] = newAddress;
				str[2] = column;
					
			break;
			
		}
		
		return str;
		
		
	}
	
	
}
